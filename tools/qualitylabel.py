...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--sort_start_int', required=False, action='store_false', help='Re-labeling mode')
    parser.add_argument('-z', '--image-size', required=False, default=512, type=int, help='Image size')
    parser.add_argument('-o', '--options-file', required=True, help='Options file (json).')
    args = parser.parse_args()

    gui = RetinaQualityLabel(
        relabel_mode= not args.relabel_mode, sort_mode= not args.sort_start_int, 
        image_size=args.image_size, options_file=args.options_file
    )
    gui.init_app()
